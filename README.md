...
As of age v1.3.0, [age supports post-quantum hybrid encryption](https://github.com/FiloSottile/age/releases/tag/v1.3.0)
using ML-KEM-768 + X25519. This provides protection against future quantum computer attacks using the
"harvest now, decrypt later" strategy.

To generate a post-quantum age key:

```console
$ age-keygen -pq -o /path/to/key.txt
```

Post-quantum keys have different prefixes:
- Public key: `age1pq1...`
- Private key: `AGE-SECRET-KEY-PQ-...`

sops-nix supports automatic generation of post-quantum keys via the `sops.age.generateKeyType` option:

```nix
{
  sops.age.keyFile = "/var/lib/sops-nix/key.txt";
  sops.age.generateKey = true;
  sops.age.generateKeyType = "hybrid-pq";  # Generate a post-quantum key instead of classic x25519
}
```

**Important:** Post-quantum age keys are fundamentally different from SSH-derived age keys:

- **PQ keys are standalone age identities** — they are NOT derived from SSH keys and cannot be used as SSH host keys.
- **`sops.age.sshKeyPaths` does NOT work with PQ keys** — the `ssh-to-age` conversion only works with classic X25519 keys derived from ed25519 SSH keys.
- **You MUST use `sops.age.keyFile`** when using PQ keys. The key file contains the raw age identity (not an SSH key).
- **PQ keys must be provisioned separately** — unlike SSH host keys which are auto-generated by `openssh`, PQ age keys need to be either:
  - Pre-provisioned on the machine before deployment, or
  - Auto-generated using `sops.age.generateKey = true` (but then you need to encrypt secrets with the generated public key after first boot)

**Note:** Post-quantum key support requires:
- age >= 1.3.0 for key generation and decryption
- sops with age >= 1.3.0 for encryption (check your sops version supports PQ keys)

**Current limitation:** As of sops v3.11.0, the bundled age library is v1.2.1 which does NOT support PQ keys for decryption. PQ key generation via `sops.age.generateKeyType = "hybrid-pq"` works, but you cannot yet use those keys to decrypt secrets with sops-nix. This will be resolved when sops updates to age >= 1.3.0.

For long-term secrets that need protection against future quantum attacks, consider using post-quantum keys.
